...
const spreadsheetId = '1RScZkEir3Jt_4EbQES5KP1m0rzUlwEkJsDHqXf5B_us';
...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        const { name, email, phone, position, remarks, resumeLink } = req.body;

        if (!name || !email || !phone || !position || !remarks || !resumeLink) {
        ...
        try {
            ...
            const response = await sheets.spreadsheets.values.get({
                spreadsheetId,
                range: 'Sheet1!A:G', // A=date, B=name, C=phone, D=email, E=position, F=remarks, G=resumeLink
            });

            const rows = response.data.values || [];

            // Find if email exists
            const emailRow = rows.find(row => row[3] === email); // column D = email
            ...
            const mailOptions = {
                ...
                subject: `New Career Form Submission – Allastir Private Limited`,
                html: `
          <div style="font-family: Arial, sans-serif; color: #333; padding: 20px 0;">
            <h2 style="color: #000; font-size: 18px; margin-bottom: 5px;">
              New Career Form Submission – Allastir Private Limited
            </h2>
            <hr>
            <p style="font-size: 14px;">
              A career application has been submitted on your website:
            </p>
            <ul>
              ...
              <li><strong>Resume:</strong> <a href="${resumeLink}" target="_blank">View resumeLink</a></li>
            </ul>
            <hr />
            <p>Regards,</p>
            <p><b>Automated Career Form</b></p>
